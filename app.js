...
function handleReceipt(file){if(!file)return;selectedReceiptFile=file;selectedReceipt={name:file.name,type:file.type||'archivo',size:file.size};$('attachment-status').textContent=file.type==='application/pdf'?'PDF listo para analizar':'Imagen lista para analizar';$('analyze-btn').disabled=false;$('attachment-preview').classList.remove('hidden');$('attachment-preview').innerHTML=`<span class="receipt-name">📎 ${escapeHtml(file.name)}<small>${file.type==='application/pdf'?'Documento PDF':'Imagen'} · ${(file.size/1024).toFixed(0)} KB</small></span><button type="button" id="remove-attachment" aria-label="Quitar comprobante">×</button>`;$('remove-attachment').addEventListener('click',clearReceipt);}
function clearReceipt(){selectedReceipt=null;selectedReceiptFile=null;$('receipt-file').value='';$('receipt-camera').value='';$('analyze-btn').disabled=true;$('analysis-result').classList.add('hidden');$('analysis-result').innerHTML='';$('attachment-preview').classList.add('hidden');$('attachment-preview').innerHTML='';$('attachment-status').textContent='Podés adjuntar una foto o un PDF';}
...
$('take-photo-btn').addEventListener('click',()=>{const video=$('camera-video'),canvas=$('camera-canvas');if(!video.videoWidth)return;canvas.width=video.videoWidth;canvas.height=video.videoHeight;canvas.getContext('2d').drawImage(video,0,0,canvas.width,canvas.height);canvas.toBlob(blob=>{const file=new File([blob],`comprobante_camara_${Date.now()}.jpg`,{type:'image/jpeg'});handleReceipt(file);closeCamera();},'image/jpeg',.9);});
function applyAnalysis(result){const fields=result.campos||{};if(fields.comercio_destinatario&&fields.comercio_destinatario!=='Sin dato')$('description').value=fields.comercio_destinatario;if(typeof fields.importe==='number')$('amount').value=fields.importe;if(fields.fecha&&/^\d{4}-\d{2}-\d{2}$/.test(fields.fecha))$('date').value=fields.fecha;if(fields.categoria&&[...$('category').options].some(option=>option.value===fields.categoria))$('category').value=fields.categoria;if(fields.comentario&&fields.comentario!=='Sin dato')$('note').value=fields.comentario;const observations=(result.observaciones||[]).map(escapeHtml).join('<br>');const questions=(result.preguntas||[]).map(escapeHtml).join('<br>');$('analysis-result').innerHTML=`<strong>${result.estado==='listo'?'Análisis listo para revisar':'Requiere revisión'}</strong>${observations?`<small>${observations}</small>`:''}${questions?`<small>Preguntas: ${questions}</small>`:''}`;$('analysis-result').classList.remove('hidden');}
$('analyze-btn').addEventListener('click',async()=>{if(!selectedReceiptFile)return;const button=$('analyze-btn');button.disabled=true;button.textContent='Analizando…';$('analysis-result').classList.remove('hidden');$('analysis-result').textContent='El agente está leyendo el comprobante…';try{const dataUrl=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(selectedReceiptFile);});const response=await fetch('/api/analyze-receipt',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({filename:selectedReceiptFile.name,mime_type:selectedReceiptFile.type,data:dataUrl.split(',')[1]})});const payload=await response.json();if(!response.ok||!payload.ok)throw new Error(payload.error||'No se pudo analizar el comprobante.');applyAnalysis(payload.result);$('attachment-status').textContent='Análisis completado · revisá los campos';}catch(error){$('analysis-result').innerHTML=`<strong>No se pudo analizar</strong><small>${escapeHtml(error.message)}. Verificá que el servidor esté iniciado y que OPENAI_API_KEY esté configurada.</small>`;}finally{button.disabled=false;button.textContent='✦ Analizar comprobante';}});
